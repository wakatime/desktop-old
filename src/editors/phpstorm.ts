...
export default class PhpStorm extends Editor {
  ...
  private appDirectory(): string {
    ...
    switch (os.platform()) {
      case 'win32':
        pathsToCheck = pathsToCheck.map(check => `${os.homedir()}\\.PhpStorm${check}`);
  ...
  private pluginsDirectories(): string[] {
    ...
    switch (os.platform()) {
      case 'win32': {
        return pathsToCheck.map(check => `${os.homedir()}\\.PhpStorm${check}\\config\\plugins`);
      }
      case 'darwin':
        return pathsToCheck.map(
          check => `${os.homedir()}/Library/Application Support/PhpStorm${check}`,
        );
      case 'linux':
        return [''];
      default:
